...
export async function rollbackAuditLog(
  ...
  const entityId = log.entityId ?? (typeof previous.id === "string" ? previous.id : null);
  ...
  switch (log.entity) {
    case "BlogPost": {
      if (log.action === "CREATE" && entityId) {
        await prisma.blogPost.delete({ where: { id: entityId } }).catch(() => null);
    ...
    case "BookshelfItem": {
      if (log.action === "CREATE" && entityId) {
        await prisma.bookshelfItem.delete({ where: { id: entityId } }).catch(() => null);
        restored = { deleted: entityId };
        break;
      }
      if (!entityId) {
  ...
  await writeAuditLog({
    ...
    entity: log.entity,
    entityId,
    previousState: log.newState ? JSON.parse(log.newState) : null,
    newState: restored,
  });

  return { ok: true, message: `Reverted ${log.entity} to the prior snapshot.` };
}
